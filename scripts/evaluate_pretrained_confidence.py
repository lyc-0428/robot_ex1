#!/usr/bin/env python3
"""Evaluate COCO-pretrained YOLOv8 laptop/mouse detections at fixed confidences."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("reports/baseline_pretrained_yolov8s")
    )
    parser.add_argument(
        "--cache", type=Path, default=Path(".cache/pretrained_coco_evaluation")
    )
...
def build_coco_mapped_dataset(
    ...
    for image in images:
        ...
        for line_number, line in enumerate(label.read_text(encoding="utf-8-sig").splitlines(), 1):
            ...
            coco_class = 63 if dataset_class == 0 else 64
            mapped_lines.append(" ".join((str(coco_class), *fields[1:])))
    ...
    yaml_path = cache_root / f"data_{split}_coco_ids.yaml"
...
def evaluate_thresholds(
    ...
    for threshold in thresholds:
        ...
        laptop = class_counts(matrix, 63)
        mouse = class_counts(matrix, 64)
...
def write_outputs(
    ...
    metadata = {
        ...
        "batch": args.batch,
        "device": args.device,
        "coco_to_dataset_class_mapping": {"63": 0, "64": 1},
    ...
    markdown_table = "\n".join(table_lines)
    readme = f"""# Pretrained YOLOv8s confidence sweep

This is the pre-fine-tuning baseline on the `{args.split}` split.
...
Best validation confidence by micro F1: **{float(best['confidence']):.2f}**
...
def main() -> int:
    ...
    if names.get(63) != "laptop" or names.get(64) != "mouse":
        raise RuntimeError("model does not have the expected COCO laptop/mouse classes")
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is not available")

    data_yaml = build_coco_mapped_dataset(args.dataset, args.split, args.cache, names)
